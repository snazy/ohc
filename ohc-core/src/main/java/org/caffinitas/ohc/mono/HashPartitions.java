...
final class HashPartitions implements Constants
{
    ...
    long getPartitionHead(long hash)
    {
        return Uns.getLongVolatile(partitionAddressHead(hash));
    }

    void setPartitionHead(long hash, long hashEntryAdr)
    {
    ...
    long lockPartition(long hash, boolean write)
    {
    ...
    void unlockPartition(long lock, long hash, boolean write)
    {
        long partLockAdr = partitionAddressLock(hash);
        if (write)
    ...
    int partitionForHash(long hash)
    {
    ...
    private long partitionAddressHead(long hash)
    {
        return table().partitionAddressHead(hash);
    }

    private long partitionAddressLock(long hash)
    {
    ...
    static final class Table
    {
        ...
        long partitionAddressLock(long hash)
        {
            return partitionAddress(hash) + PART_OFF_LOCK;
        }

        long partitionAddressHead(long hash)
        {
            return partitionAddress(hash) + PART_OFF_PARTITION_HEAD;
        }

        private long partitionAddress(long hash)
        {
